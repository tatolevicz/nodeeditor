...
class NODE_EDITOR_PUBLIC QmlBasicGraphicsScene : public QQuickPaintedItem
{
    Q_OBJECT
public:
    QmlBasicGraphicsScene(AbstractGraphModel &graphModel, QQuickItem *parent = nullptr);

    // Scenes without models are not supported
    QmlBasicGraphicsScene() = delete;

    ~QmlBasicGraphicsScene();
